...
Notion 데이터베이스 프로퍼티 스키마 가정 (결정 필요: 실제 발행자의 기존 노션 구조에 맞춰 확정):

| 프로퍼티명 | 타입(Notion) | 설명 |
|---|---|---|
| 견적서명 | Title | 견적서 제목 |
| 클라이언트명 | Text | 수신 클라이언트 이름/회사명 |
| 공급자 정보 | Text | 발행자(공급자) 상호/사업자 정보 |
| 항목 (품목/수량/단가) | Relation 또는 Sub-DB | 견적 라인 아이템. 별도 하위 데이터베이스로 관리 가정 |
| 합계 금액 | Formula/Number | 항목 합산 금액 |
| 유효기간 | Date | 견적서 유효 기한 |
| 상태 | Select | 예: 임시저장/발행됨 — 발행됨 상태만 웹뷰 노출 (결정 필요) |

### 견적서 웹뷰

- 라우팅: `PageProps<"/quote/[id]">`를 사용하는 동적 라우트(`src/app/quote/[id]/page.tsx`)로 구현. `params`는 Promise이므로 `await`으로 해석한다.
- 표시 항목: 견적서명, 발행일, 유효기간, 공급자/클라이언트 정보, 항목별 품목·수량·단가·소계, 합계 금액
...
|---|---|---|---|
| 견적서 상세(웹뷰) | `/quote/[id]` | 견적서 헤더(발행자/클라이언트 정보), 항목 테이블, 합계 영역, PDF 다운로드 버튼 | 로딩: 스켈레톤 UI / 에러: "견적서를 찾을 수 없습니다" 안내 + 홈 링크 / 빈 데이터: 항목 없음 안내 |
...
- [ ] 성공 지표별 구체적 목표 수치 (1장)
- [ ] 노션 데이터베이스 실제 프로퍼티 스키마 확정 (4장)
- [ ] 항목(품목/수량/단가) 데이터를 별도 하위 DB로 관리할지, 단일 텍스트/JSON 필드로 관리할지 (4장)
- [ ] PDF 생성 방식 최종 확정 — 브라우저 인쇄 vs 서버사이드 렌더링 (4장)
- [ ] 접근 제어 수준 최종 확정 — 완전 공개 vs 랜덤 토큰 vs 만료 서명 링크 (4장)
- [ ] on-demand revalidation(노션 수정 즉시 반영) 도입 여부 (5장)
- [ ] 다중 통화 지원 여부 (2장)
- [ ] 랜딩 페이지 포함 여부 (7장)
- [ ] 배포 환경(Vercel 등) 및 이에 따른 PDF 생성 방식 제약 확인
